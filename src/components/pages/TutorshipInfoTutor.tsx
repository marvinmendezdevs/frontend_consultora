...
import type { TutorInfoViewType } from "@/types/tutorship.types";

function TutorshipInfoTutor() {
    const { username } = useParams();
    const { isLoading, isError, data } = useQuery<TutorInfoViewType>({
        queryKey: ["tutorship-info"],
        queryFn: () => {
            if(!username){
                throw new Error('tutor no encontrado');
            }
            return getTutorshipInfo(username)
        },
        retry: false,
    ...
    if(data){
        const diagnosticoCount = data.responses.reduce((acc, item) => item.instrumentId === 1 ? acc + 1 : acc, 0);
        const observacionCount = data.responses.reduce((acc, item) => item.instrumentId === 2 ? acc + 1 : acc, 0);
